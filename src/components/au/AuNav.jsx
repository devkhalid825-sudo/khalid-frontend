...
import { useAuTheme } from './AuThemeProvider';
...
const AuNav = ({
  ...
  const { theme, toggleTheme } = useAuTheme();
  return (
    <header className="au-nav">
      <nav className="au-container au-nav-inner">
        ...
        <div className="au-nav-links">
          ...
          {phoneBadge && <span className="au-nav-phone">{phoneBadge}</span>}
        </div>
        <div className="au-nav-right">
          <button type="button" className="au-theme-toggle" aria-label="Toggle theme" onClick={toggleTheme}>
            {theme === 'dark' ? (
          ...
          <Link href={ctaHref} className="au-btn au-btn-primary au-btn-sm">{cta}</Link>
        </div>
      </nav>
    </header>
  );
